#include <iostream>
#include <WS2tcpip.h>

using namespace std;
#pragma comment (lib, "WS2_32.LIB")

const char* SERVER_ADDR = "127.0.0.1";
const short SERVER_PORT = 4000;
const int BUFSIZE = 256;

char recv_buf[BUFSIZE];
SOCKET s_socket;
WSABUF mybuf_r;
WSABUF s_wsabuf;
char send_buf[BUFSIZE];

void do_recv(SOCKET s_socket);

void error_display(const char* msg, int err_no)
{
	WCHAR* lpMsgBuf;
	FormatMessage(
		FORMAT_MESSAGE_ALLOCATE_BUFFER |
		FORMAT_MESSAGE_FROM_SYSTEM,
		NULL, err_no,
		MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
		(LPTSTR)&lpMsgBuf, 0, NULL);
	std::cout << msg;
	std::wcout << L"에러 " << lpMsgBuf << std::endl;
	while (true);
	LocalFree(lpMsgBuf);
}

void CALLBACK recv_callback(DWORD err, DWORD num_byte, LPWSAOVERLAPPED lp_over, DWORD s_flag)
{
	cout << "Server Sent [" << num_byte << "bytes] : " << recv_buf << endl;
	delete lp_over;

	do_recv(s_socket);
	return;
}

void CALLBACK send_callback(DWORD err, DWORD num_byte, LPWSAOVERLAPPED lp_over, DWORD s_flag)
{
	delete lp_over;
	return;
}

void do_recv(SOCKET s_socket)
{
	//char recv_buf[BUFSIZE];
	//WSABUF mybuf_r;
	mybuf_r.buf = recv_buf; mybuf_r.len = BUFSIZE;
	//DWORD recv_byte;
	DWORD recv_flag = 0;
	WSAOVERLAPPED* r_over = new WSAOVERLAPPED;
	ZeroMemory(r_over, sizeof(WSAOVERLAPPED));
	int ret = WSARecv(s_socket, &mybuf_r, 1, 0, &recv_flag, r_over, recv_callback);
	if (ret != 0) {
		int err_no = WSAGetLastError();
		if (err_no != WSA_IO_PENDING)
			error_display("WSARecv : ", err_no);
	}
	//cout << "Server Sent [" << recv_byte << "bytes] : " << recv_buf << endl;
}

int main()
{
	wcout.imbue(locale("korean"));
	WSADATA WSAData;
	WSAStartup(MAKEWORD(2, 0), &WSAData);
	s_socket = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, 0, 0, WSA_FLAG_OVERLAPPED); // 소켓 플래그 바뀜
	SOCKADDR_IN server_addr;
	ZeroMemory(&server_addr, sizeof(server_addr));
	server_addr.sin_family = AF_INET;
	server_addr.sin_port = htons(SERVER_PORT);
	inet_pton(AF_INET, SERVER_ADDR, &server_addr.sin_addr);
	connect(s_socket, reinterpret_cast<sockaddr*>(&server_addr), sizeof(server_addr));

	do_recv(s_socket);	// 먼저 리시브해서 등록

	for (;;) {
		//char send_buf[BUFSIZE];
		cout << "Enter Message : "; cin.getline(send_buf, BUFSIZE);
		DWORD sent_byte;
		//WSABUF s_wsabuf;
		s_wsabuf.buf = send_buf; s_wsabuf.len = static_cast<ULONG>(strlen(send_buf)) + 1;
		WSAOVERLAPPED* s_over = new WSAOVERLAPPED;
		ZeroMemory(s_over, sizeof(WSAOVERLAPPED));
		WSASend(s_socket, &s_wsabuf, 1, 0, 0, s_over, send_callback);
		SleepEx(1000, true); // 비동기식이라 언제 올지 모름, 게임에 어울림 코드가 단순해서 못받음
	}
	WSACleanup();
}